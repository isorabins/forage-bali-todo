...
// ── Accomplishment labels per week — shown near stars ─────────────────────────
...
function buildConstellation(): { stars: Star[]; connections: Connection[] } {
  ...
  for (let w = 1; w <= 12; w++) {
    ...
    // Label indices: assign to stars at positions 1, 4, 7 (spread across the cluster)
    const labelSlots = [1, 4, 7]
    ...
    for (let i = 0; i < STARS_PER_WEEK; i++) {
      ...
      // Assign a label to this star if it's in a label slot
    ...
    <div style={{
      ...
      >
        ...
        {ALL_STARS.map((star, i) => {
          ...
          const labelAlpha  = isCurrent ? 0.9 : Math.max(0.18, 0.65 - age * 0.05)
            ...
            <g key={`star${i}`}>
              ...
              {star.label && (
                <text
                  x={star.x + labelOffX}
                  y={star.y + 4}
                  fill={`rgba(180,190,220,${labelAlpha})`}
                  fontSize={10}
                  fontFamily="'Inter', system-ui, sans-serif"
